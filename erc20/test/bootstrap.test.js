...
contract('Governor replacement', (accounts) => {
    ...
    it("should pass setup", async () => {
        ...
        governor = await Governor_Owned.at(govAddr)
    })
    ...
    it("should allow bootstrapping: owner -> VoteOne", async () => {
        ...
        let newGovernor = await Governor_Any_Owned.new(proxy.address, accounts[0])

        // Ask old governor to update the governor
        await truffleAssert.passes(governor.updateGovernor(newGovernor.address))
    ...
    it("should allow bootstrapping: VoteOne -> VoteOne NoLeader (as voter 2)", async () => {
        let newGovernor = await Governor_Any.new(proxy.address)

        await truffleAssert.passes(governor.updateGovernor(newGovernor.address, {from: accounts[2]}))
    ...
    it("should allow bootstrapping: VoteOne NoLeader -> VoteAll (as voter 3)", async () => {
        let newGovernor = await Governor_Cohort.new(proxy.address, 100)
        
        await truffleAssert.passes(governor.updateGovernor(newGovernor.address, {from: accounts[3]}))

        governor = newGovernor
    })

    it("should allow adding voters to VoteAll", async () => {
        await truffleAssert.passes(governor.modifyVoter(accounts[0], true))
        await truffleAssert.passes(governor.modifyVoter(accounts[1], true))
        await truffleAssert.passes(governor.modifyVoter(accounts[2], true))
        await truffleAssert.passes(governor.modifyVoter(accounts[3], true))
    })
    ...
    it("should allow bootstrapping: VoteAll -> VoteAll NoLeader", async () => {
        let newGovernor = await Governor_Cohort_Consortium.new(proxy.address, 100, [accounts[0], accounts[1], accounts[2], accounts[3]])
        ...
        // Now actually do it
        await truffleAssert.fails(governor.updateGovernor(newAddress, {from: executor}))

        // Oops forgot a vote
        await truffleAssert.passes(governor.vote(action, executor, newAddress, {from: accounts[3]}))

        // Try again
        await truffleAssert.passes(governor.updateGovernor(newAddress, {from: executor})) 
    ...
    it("should allow replacing the logic address (with VoteAll)", async () => {
        ...
        // Now actually do it
        await truffleAssert.fails(governor.updateLogic(newAddress, {from: executor}))

        // Oops forgot a vote
        await truffleAssert.passes(governor.vote(action, executor, newAddress, {from: accounts[3]}))

        // Try again
        await truffleAssert.passes(governor.updateLogic(newAddress, {from: executor}))
